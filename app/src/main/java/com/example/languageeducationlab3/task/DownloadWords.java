...
public class DownloadWords {
    ...
    private void loadWordsFromFile(Resources resources) {
        ...
        this.englishWords = IntStream.range(0, wordsEng.length).boxed().collect(Collectors.toMap(i -> wordsEng[i], i -> translateEng[i]));

        this.germanWords = IntStream.range(0, wordsGerm.length).boxed().collect(Collectors.toMap(i -> wordsGerm[i], i -> translateGerm[i]));
